...
(function ($) {
    function SocialTimeline(element, options) {
        ...
        for (x in this.settings.feeds)
        {
            ...
            if (typeof feeds[x].limit !== 'undefined') {
                try {
                    if (isNaN(feeds[x].limit)) {
                        this.feeds[x + '_limit'] = feeds[x].limit.split(',');
                    } else {
                        this.feeds[x + '_limit'] = feeds[x].limit;
                    }
                } catch (err) {
                }
            }
            if (typeof this.feeds[x + '_limit'] === 'undefined') {
                this.feeds[x + '_limit'] = 0;
            }
    ...
    SocialTimeline.prototype = {
                ...
                this._parseRSS(this.social[num].url[multi_counter], this.social[num].limit, multi_counter,
                        function (feeds, id) {
                            ...
                            for (var i = 0; i < feeds.entries.length; i++) {
                                ...
                                if (feeds.entries[i].author == "" || feeds.entries[i].author == "Webstagram") {
                                    feeds.entries[i].author = me.social[num].name[id];
                                }
            ...
            if (this.settings.showFilter && this.totalFeeds > 1) {
                ...
                for (var i = 0; i <= this.totalFeeds; i++) {
                    if (typeof this.nameFeeds[i] !== "undefined" && $.inArray(this.nameFeeds[i], oldNameFeeds) === -1) {
                        ...
                        if (typeof this.iconFeeds[i] != "undefined" && this.iconFeeds[i] != "") {
                            $('.' + (this.nameFeeds[i]), outputOptions.$dpSocialTimelineFilter).css('background-image', 'url(' + this.iconFeeds[i] + ')')
                        }
        ...
        _createMarkup: function (outputOptions, num, afterLoad) {
            ...
            for (x = num; x < this.entry.length; x++)
            {
                ...
                if (this.settings.showSocialIcons) {
                    ...
                    if (typeof this.entry[x].icon != "undefined" && this.entry[x].icon != "") {
                        $($favicon).css('background-image', 'url(' + this.entry[x].icon + ')')
                    }
                } else {
                    $favicon = '';
                }
                ...
                var $tw_btn = jQuery('<a />')
                        ...
                        .click(function (event) {
                            ...
                            $instabtn=jQuery('<a />').addClass('share_instagram')
                            .attr({href: 'http://sociallabels.imnwebhosting.com/share/', title: 'Share in Instagram'})
                        .click(function (event) {
                            var width = 575,
                                    height = 400,
                                    left = (jQuery(window).width() - width) / 2,
                            ...
                            $flickrbtn=jQuery('<a />').addClass('share_flickr')
                            .attr({href: 'http://sociallabels.imnwebhosting.com/share/flickr.php?img='+btoa(simg), title: 'Share in Flickr'})
                        .click(function (event) {
                            var width = 575,
                                    height = 400,
                                    left = (jQuery(window).width() - width) / 2,
                         ...
                         $tw_btn = jQuery('<a />')
                        .addClass('share_twitter')
                            .attr({href: 'http://sociallabels.imnwebhosting.com/share/twitter.php?img='+btoa(simg), title: 'Share in Twitter'})
                        .click(function (event) {
                            var width = 575,
                                    height = 400,
                                    left = (jQuery(window).width() - width) / 2,
                        ...
                        $fb_btn = jQuery('<a />')
                        .addClass('share_facebook')
                            .attr({href: 'http://sociallabels.imnwebhosting.com/share/facebook.php?img='+btoa(simg), title: 'Share in Facebook'})
                        .click(function (event) {
                            var width = 575,
                                    height = 400,
                                    left = (jQuery(window).width() - width) / 2,
                ...
                $stContentFoot = $('<div />').addClass('dpSocialTimelineContentFoot')
                        ...
                        .append((this.settings.share ? $($flickrbtn) : ''))
                        .append((this.settings.share ? $($instabtn) : ''));

                outputOptions.$dpSocialTimelineContent.append($("<div />").css('clear', 'both')).append($($stContentFoot));
        ...
        _onLoadHook: function (outputOptions) {
            ...
            jQuery('.dpSocialTimeline_noImage').parent('.dpSocialTimeline_item').remove();
                ...
                loadingCompleteCallback: function () {
                    ...
                    $(outputOptions.$div).isotope({
                        ...
                        onLayout: function ($elems, instance) {
                            ...
                            $('span', outputOptions.$dpSocialTimelineLine).fadeOut('fast', function () {
                                $(this).remove();
                            });
        ...
        _parseRSS: function (url, limit, int_val, fnk, num) {
            ...
            if (limit != null && limit > 0 && typeof limit != 'undefined')
                gurl += "&num=" + limit;
            else if ($.isArray(limit))
                gurl += "&num=" + limit[int_val];
            else if (num != null)
                gurl += "&num=" + num;
    ...
    $.fn.dpSocialTimeline.defaults = {
        ...
        oneColumnItemWidth: '',
        skin: 'light',
        total: 500 // Total items to retrieve
    };
